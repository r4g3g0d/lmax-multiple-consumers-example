...
@Service
public class DisruptorListenerForComputation implements EventHandler<RingBufferEvent> {

    private long ordinal;
    private long noOfConsumers;

    /*public DisruptorListenerForComputation(long ordinal, long noOfConsumers) {
    ...
    public void onEvent(RingBufferEvent ringBufferEvent, long l, boolean b) throws Exception {
        if ((l % noOfConsumers) == ordinal) {
            System.out.println(
                    "[DisruptorListenerForComputation] - " + ringBufferEvent.getId() + " Time : " + LocalDateTime
                    .now());
        }
    }
