...
@Service
public class DisruptorListener implements EventHandler<RingBufferEvent> {

    private long ordinal;
    private long noOfConsumers;

    /*public DisruptorListener(long ordinal, long noOfConsumers) {
    ...
    public void onEvent(RingBufferEvent ringBufferEvent, long l, boolean b) throws Exception {
        if ((l % noOfConsumers) == ordinal) {
            System.out.println("[DisruptorListener] - " + ringBufferEvent.getId() + " Time : " + LocalDateTime
                    .now());
        }
    }
